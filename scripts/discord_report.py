...
for page in query.get("results", []):
    ...
    status = props["업무 항목 현황"]["status"]["name"] if props["업무 항목 현황"]["status"] else "-"
    if status in DONE_STATUSES:
        continue

    title_list = props["업무 항목"]["title"]
    name = title_list[0]["plain_text"] if title_list else "(제목 없음)"

    date_prop = props["작업일정"]["date"]
    ...
    area = props["업무 영역"]["select"]["name"] if props["업무 영역"]["select"] else "-"
